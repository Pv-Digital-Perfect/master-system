...
export default function OfferRequest() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success("PV-Anfrage wurde gespeichert.");
      navigate("/danke");
    } catch (error) {
      console.error(error);
      toast.error("Die Anfrage konnte nicht gespeichert werden. Bitte prüfe Supabase-Tabelle, neue Spalten und RLS.");
    } finally {
        ...
        <section className="relative overflow-hidden bg-white py-16 md:py-24">
          ...
          <div className="container relative mx-auto px-4">
            <div className="mx-auto max-w-3xl text-center">
              <div className="mb-5 inline-flex rounded-full border border-[#808080]/20 bg-[#F8FAFC] px-4 py-2 text-xs font-extrabold uppercase tracking-[0.18em] text-[#F97316]">Qualifizierte PV-Anfrage</div>
              <h1 className="font-display text-4xl font-extrabold tracking-[-0.045em] md:text-6xl">Photovoltaik-Angebot anfordern</h1>
              <p className="mt-5 text-lg leading-relaxed text-slate-600">Die Anfrage sammelt genau die Daten, die ein PV-Anbieter für eine schnelle Ersteinschätzung braucht — mehr Leadqualität, weniger Rückfragen.</p>
            </div>
          </div>
        </section>
          ...
          <Card className="mx-auto -mt-10 max-w-6xl border border-[#808080]/20 bg-white shadow-2xl shadow-slate-900/10">
            <CardContent className="p-6 md:p-10">
              <form onSubmit={handleSubmit} className="space-y-9">
                <FormBlock title="Kontakt" description="Damit der Anbieter den Lead schnell erreichen kann.">
                ...
                <FormBlock title="Objekt & Dach" description="Diese Daten machen aus einer Anfrage einen verwertbaren PV-Lead.">
                ...
                <FormBlock title="Verbrauch & Technik" description="Speicher, Wallbox und Zählerschrank entscheiden stark über Aufwand und Nutzen.">
                  <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <Field label="Wallbox interessant?"><Select value={form.wallbox_interest} onValueChange={(value) => update("wallbox_interest", value)}><SelectTrigger><SelectValue placeholder="Bitte wählen" /></SelectTrigger><SelectContent><SelectItem value="yes">Ja</SelectItem><SelectItem value="no">Nein</SelectItem><SelectItem value="later">Vielleicht später</SelectItem></SelectContent></Select></Field>
                ...
                <div className="flex flex-col gap-4 rounded-3xl border border-[#808080]/20 bg-[#F8FAFC] p-5 md:flex-row md:items-center md:justify-between">
                  <div className="flex gap-3 text-sm text-slate-600"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#22C55E]" /><span>Öffentliche Besucher dürfen Leads nur erstellen. Lesen und Bearbeiten bleibt Admin-Sache — RLS bleibt Pflicht.</span></div>
